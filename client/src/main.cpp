...
// Print the result of an operation.  For file-mutating calls we also show the
// authoritative version the server echoed back in the ACK.
static void checkResponse(const std::string &op, const Message &msg)
{
    if (msg.isAck()) {
        // FILE_VERSION is present in ACKs for create/read/update operations.
