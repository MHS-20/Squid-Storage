...
Server::Server(int port, int replicationFactor, int timeoutSeconds) {
  ...
  dataNodeReplicationMap =
      map<string, map<string, shared_ptr<ConnectionSession>>>();
}
...
void Server::handleClientRequest(ConnectionSession &clientSession,
    ...
    bool ok =
        requestPool_
            .submit([this, filePath, fileVersion,
              ...
              return propagateCreateFile(filePath, fileVersion, origin,
                                         fileData);
            })
            .get();
  ...
  case Opcode::UPDATE_FILE: {
    ...
    bool ok =
        requestPool_
            .submit([this, filePath, fileVersion,
                     origin = clientSession.getProcessName(), fileData]() {
              return propagateUpdateFile(filePath, fileVersion, origin,
                                         fileData);
            })
            .get();
  ...
  case Opcode::SYNC_STATUS:
    clientSession.response(
        requestPool_.submit([this]() { return getFileVersionMap(); }).get());
    break;
  case Opcode::ACQUIRE_LOCK:
    clientSession.response(
        requestPool_
            .submit([this, filePath]() { return acquireLock(filePath); })
            .get());
    break;
  case Opcode::RELEASE_LOCK:
    clientSession.response(
        requestPool_
            .submit([this, filePath]() { return releaseLock(filePath); })
            .get());
    break;
...
void Server::handleAccept(AcceptedConnection accepted) {
  ...
  if (peerNodeType == "DATANODE") {
    auto datanodeSession = std::make_shared<ConnectionSession>(
  ...
  auto clientSession = std::make_shared<ConnectionSession>(
  ...
  while (clientProtocol.isAlive()) {
    ...
    case Opcode::CREATE_FILE: {
      ...
      if (clientProtocol.receiveFileData(fileData)) {
        bool ok = propagateCreateFile(
            filePath, fileVersion, clientProtocol.getProcessName(), fileData);
...
bool Server::acquireLock(string path) {
  ...
  if (!fileLockMap[path].isLocked()) {
    ...
    fileLockMap[path].setExpiration(chrono::system_clock::now() +
...
void Server::buildFileLockMap() {
  ...
  for (auto &datanodeSession : datanodes) {
    ...
    for (auto &file : fileMap) {
      ...
      dataNodeReplicationMap[file.first][datanodeSession->getProcessName()] =
          datanodeSession;
    }
  }
}
...
void Server::eraseFromReplicationMap(string datanodeName) {
  vector<std::pair<string, map<string, std::shared_ptr<ConnectionSession>>>>
      rebalanceTargets;
  {
    std::unique_lock<std::shared_mutex> lock(stateMutex);
    for (auto it = dataNodeReplicationMap.begin();
         it != dataNodeReplicationMap.end(); ++it) {
...
void Server::rebalanceFileReplication(
    string filePath,
  ...
  for (auto &candidate : candidates) {
    Message response = candidate.second->createFile(
        filePath, fileManager_.getFileVersion(filePath),
        fileData);
    if (response.isAck()) {
...
bool Server::propagateCreateFile(string filePath, int version,
  ...
  for (auto &client : clients) {
    if (!client.second || client.first == originProcessName)
      continue;
    client.second->post([filePath, version, fileData](SquidProtocol &protocol) {
      protocol.createFile(filePath, version, fileData);
    });
  }
...
bool Server::propagateUpdateFile(string filePath, int version,
                                 const string &originProcessName,
                                 const vector<uint8_t> &fileData) {
  vector<pair<string, std::shared_ptr<ConnectionSession>>> datanodes;
  vector<pair<string, std::shared_ptr<ConnectionSession>>> clients;
  ...
  for (auto &client : clients) {
    if (!client.second || client.first == originProcessName)
      continue;
    client.second->post([filePath, version, fileData](SquidProtocol &protocol) {
      protocol.updateFile(filePath, version, fileData);
    });
  }
...
void Server::propagateDeleteFile(string filePath,
                                 const string &originProcessName) {
  vector<pair<string, std::shared_ptr<ConnectionSession>>> datanodes;
  vector<pair<string, std::shared_ptr<ConnectionSession>>> clients;
...
void Server::propagateCreateFile(string filePath,
                                 const string &originProcessName) {
  (void)propagateCreateFile(filePath,
                            fileManager_.getFileVersion(filePath),
                            originProcessName, {});
}

void Server::propagateCreateFile(string filePath, int version,
                                 const string &originProcessName) {
  (void)propagateCreateFile(filePath, version, originProcessName, {});
}

void Server::propagateUpdateFile(string filePath,
                                 const string &originProcessName) {
  (void)originProcessName;
  (void)filePath;
}

void Server::propagateUpdateFile(string filePath, int version,
                                 const string &originProcessName) {
  (void)originProcessName;
  (void)version;
  (void)filePath;
...
void Server::printMap(map<string, std::shared_ptr<ConnectionSession>> &map,
                      string name) {
  cout << "[SERVER]: " << name << endl;
  for (auto &pair : map)
    cout << pair.first << " => "
         << (pair.second ? pair.second->toString() : "<null>") << endl;
}

void Server::printMap(map<string, FileLock> &map, string name) {
  cout << "[SERVER]: " << name << endl;
  for (auto &pair : map)
    cout << pair.first << " => " << pair.second.getFilePath() << " : "
         << pair.second.isLocked() << endl;
}

void Server::printMap(
    map<string, map<string, std::shared_ptr<ConnectionSession>>> &map,
    string name) {
  cout << "[SERVER]: " << name << endl;
  for (auto &pair : map) {
    cout << pair.first << " => ";
    for (auto &innerPair : pair.second)
      cout << innerPair.first << " : "
           << (innerPair.second ? innerPair.second->toString() : "<null>")
           << ", ";
    cout << endl;
  }
}
...
Server::getDataNodeSessionLocked(const std::string &name) {
  std::shared_lock<std::shared_mutex> lock(stateMutex);
...
std::shared_ptr<ConnectionSession>
Server::getClientSessionLocked(const std::string &name) {
  std::shared_lock<std::shared_mutex> lock(stateMutex);
